...
class Qwen2DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        l2 = torch.norm(hidden_states, dim=-1).detach().cpu()

        # ---- 2. average over batch ----
        l2_mean = l2.mean(dim=0)  # shape [S]

        # ---- 3. plot ----
        plt.figure(figsize=(10, 4))
        plt.plot(l2_mean.numpy())
        plt.title(f'image_{self.layer_idx}')
        plt.xlabel("Sequence Position (token index)")
        plt.ylabel("L2 Norm")
        plt.grid(True)
        plt.savefig('sink_image.png')
        breakpoint()
